...
MIN_CONFIDENCE     = 65     # % — এর নিচে কোনো match "real" ধরা হবে না
TOLERANCE          = 1 - (MIN_CONFIDENCE / 100)   # face_recognition distance-এ কনভার্ট (0.35)
MAX_IMAGE_SIZE     = (800, 800)
WP_BASE_URL        = os.getenv("WP_BASE_URL",        "https://modelsaler.com")
WC_CONSUMER_KEY    = os.getenv("WC_CONSUMER_KEY",    "")
WC_CONSUMER_SECRET = os.getenv("WC_CONSUMER_SECRET", "")
BUILD_SECRET       = os.getenv("BUILD_SECRET",       "build123")
# Render web services automatically set RENDER_EXTERNAL_URL to the public URL
SELF_URL            = os.getenv("RENDER_EXTERNAL_URL", "https://friendly-face.onrender.com")
...
# ----------------------------------------------------------
# SELF-PING — free instance ঘুমিয়ে (spin down) যাওয়া আটকাতে
# প্রতি ১০-১৪ মিনিট পরপর নিজেকেই ping করে
# ----------------------------------------------------------
async def self_ping_loop():
    await asyncio.sleep(60)  # সার্ভার পুরোপুরি startup হতে সময় দেওয়া হলো
    while True:
        wait_seconds = random.randint(600, 840)  # 10-14 minutes
        await asyncio.sleep(wait_seconds)
        try:
            await asyncio.to_thread(requests.get, f"{SELF_URL}/health", timeout=15)
            print(f"Self-ping OK ({wait_seconds}s gap) -> {SELF_URL}/health")
        except Exception as e:
...
def download_db(secret: str):
    """
    Build সম্পূর্ণ হওয়ার পর এই URL visit করলে face_database.json ফাইলটা download হবে।
    এই ফাইলটা GitHub repo-তে (main.py-এর পাশেই) commit করে রাখলে, পরবর্তী যেকোনো
    কোড-change deploy-তে এটা আগে থেকেই থাকবে — নতুন করে ৩+ ঘণ্টার build আর লাগবে না।
    শুধু মডেল/ছবি পরিবর্তন হলেই আবার /build-db চালিয়ে নতুন ফাইল download+commit করে দিতে হবে।
    """
    if secret != BUILD_SECRET:
        raise HTTPException(status_code=403, detail="Wrong secret key.")
    if not os.path.exists(DB_PATH):
        raise HTTPException(status_code=404, detail="face_database.json not found — আগে /build-db চালাও।")
    return FileResponse(DB_PATH, media_type="application/json", filename="face_database.json")
...
async def search_face(file: UploadFile = File(...)):
    ...
    for product in face_db:
        ...
        if best_dist < TOLERANCE:
            results.append((best_dist, product))
    gc.collect()

    results.sort(key=lambda x: x[0])
    top_results = results[:3]   # সর্বোচ্চ ৩টা model দেখাবে, confidence অনুযায়ী sorted (সবচেয়ে বেশি প্রথমে)

    if top_results:
        matches = []
        for dist, product in top_results:
